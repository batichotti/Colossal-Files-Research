...
repositories = pl.read_csv(repositories_path)
...
def process_repository(i):
    ...
    repository, language = repositories[i, ['url', 'main language']]
    ...
    if path.exists(f"{large_files_commits_path}{repo_path}"):
        ...
        large_files_commits_df = pl.DataFrame({"hash": hashs_large, "files": files_large})
        large_files_commits_df.write_csv(f"{output_path}large_files/{repo_path}.csv")
    ...
    if path.exists(f"{small_files_commits_path}{repo_path}"):
        ...
        small_files_commits_df = pl.DataFrame({"hash": hashs_small, "files": files_small})
        small_files_commits_df.write_csv(f"{output_path}small_files/{repo_path}.csv")
...
large_files_commits_total_df = pl.DataFrame({"hash": large_files_commits_total})
large_files_commits_total_df.write_csv(f"{output_path}large_files.csv")

small_files_commits_total_df = pl.DataFrame({"hash": small_files_commits_total})
small_files_commits_total_df.write_csv(f"{output_path}small_files.csv")

commits_total = list(set(large_files_commits_total + small_files_commits_total))
commits_total_df = pl.DataFrame({"hash": commits_total})
commits_total_df.write_csv(f"{output_path}commits_total.csv")
...
summary_df.write_csv(f"{output_path}summary.csv")
