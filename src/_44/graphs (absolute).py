...
input_path: str = "./src/_44/input/"
output_path = "./src/_44/output/"
            ...
            for large_file_name in large_file_list["path"].apply(lambda x: x.split("/")[-1]).values:
                ...
                if path.exists(f"{large_filtered_path}{repo_path}/{large_file_name}.csv"):
                    ...
                    if not large_file_data.empty:
                        ...
                        # precisa do percentil 99 válido
                        ...
                        ax.plot(df_plot[time_col], df_plot[nloc_col], 
                               marker="o", markersize=4, linewidth=1.5, 
                               color='#2E86AB', label="NLOC")

                        # linha fixa do p99
                        ax.axhline(p99, color='#A23B72', linestyle='--', 
                                  linewidth=2, label=f"p99 = {int(p99)} LOC")

                        # FIXAR LIMITES DO EIXO Y
                        max_value = df_plot[nloc_col].max()
                        y_max = max(max_value, p99) * 1.1
                        ...
                        ax.set_ylabel("Lines of Code (LOC)", fontsize=12, fontweight='normal')
