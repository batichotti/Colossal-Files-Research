...
def grew_or_decreased(repository_commits: pd.DataFrame, change_type: str = "large") -> pd.DataFrame:
    ...
    result: dict = {
        ...
        "%% Grow": [len(balances_grow)/added_files_total if added_files_total > 0 else 0],
        "%% Decrease": [len(balances_decreased)/added_files_total if added_files_total > 0 else 0],
        "%% Zero": [len(balances_zero)/added_files_total if added_files_total > 0 else 0],
        ...
        "Decreased Max": [np.max(balances_decreased) if balances_decreased else 0],
        ...
        "Decreased Large Max": [np.max(balances_large_decreased) if balances_large_decreased else 0],
        ...
        "Decreased Small Max": [np.max(balances_small_decreased) if balances_small_decreased else 0],
